...
# ✅ FIXED BASE PATH (important for Vercel)
BASE_DIR = Path(__file__).resolve().parent.parent
DATA_DIR = BASE_DIR / "data"
TEMPLATE_DIR = BASE_DIR / "templates"
STATIC_DIR = BASE_DIR / "static"
...
def _load_history() -> List[Dict[str, Any]]:
    # ❌ Disabled for Vercel (read-only filesystem safe fallback)
    return []


def _append_history(entry: Dict[str, Any]) -> None:
    # ❌ Disabled (Vercel doesn't support file writing)
    pass
...
def create_app() -> Flask:
    ...
    groq_api_key = os.getenv("GROQ_API_KEY")
    if not groq_api_key:
        return Flask(__name__)  # fallback to avoid crash
    ...
    def predict():
        try:
            ...
            context = "\n".join([
                ...
                f"Precautions: {', '.join(d.get('precautions', []))}\n"
                f"OTC Medicines: {', '.join([m['name'] for m in d.get('otc_medicines', [])])}\n"
                f"When to consult doctor: {', '.join(d.get('when_to_consult_doctor', []))}"
                for d in diseases
            ])

            prompt = f"""You are a healthcare chatbot. Use the following disease information as context.
...
Provide suggestions, precautions, and when to see a doctor.
Include: "This is not a medical diagnosis tool."
"""
